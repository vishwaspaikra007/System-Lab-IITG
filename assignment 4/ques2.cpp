...
void grade(int nQues, char ansKey[], ansBuff ans[]) //function to calculate and display the grade
{
	int total = 0;
	printf("******Grading******\n100 to 90 - A\n89 to 80 - B\n79 to 70 - C\n69 to 30 - D\nLess than 30 - FF\n*******************\n");
	printf("ans key :%s\n", ansKey);
...
{
	int msqid1, msqid2;
    key_t key = 131; //129 		//unique token for every message queue
	key_t key2 = 130; //128		
	...
	if(msgsnd(msqid1, &qSBuf, MSGSZ*noOfQues, IPC_NOWAIT) < 0) //sending the questions to child process
	{
		perror("msgsnd123");
		exit(1);
	}
	...
	for(int i = 0; i < noOfStudents; i++)
	{
		...
		if(cPid == 0)
		{
			srand(time(NULL));
			cout <<"\n";
			if(msgrcv(msqid1, &qRBuf, MSGSZ*noOfQues, 0, 0) < 0) 
				{
					perror("msgrcv");
					exit(1);
				}
			...
			if(msgsnd(msqid2, &aSBuff, noOfQues*MSGSZ, IPC_NOWAIT) < 0) // sending the random answers to parent
			{
		...
		{
			sleep(1);
			if(msgrcv(msqid2, &aRBuff, noOfQues*MSGSZ, 0, 0) < 0) //parent receiving the ans in aRBuff
			{
				perror("msgrcv111");
				exit(1);
			}
			...
			if(msgsnd(msqid1, &qSBuf, MSGSZ*noOfQues, IPC_NOWAIT) < 0) 
			{
